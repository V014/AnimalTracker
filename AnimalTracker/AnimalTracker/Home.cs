...
namespace AnimalTracker
{
    ...
    {
        private SQLiteConnection con = new SQLiteConnection ("Data Source=Animaldb.db;Version=3;New=Flase;Compress=True;");
        ...
        private void materialTabControl_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (materialTabControl.SelectedTab == physique_page)
            {
                ...
                if (physiqueTabs.SelectedTab == chart_page)
                {
                    ...
                    weight_chart.Series[0].XValueMember = "AnimalId";
                    weight_chart.Series[0].YValueMembers = "AnimalId";

                    weight_chart.Series[1].XValueMember = "Weight";
                    weight_chart.Series[1].YValueMembers = "Weight";

                    // pull the weight Id to reference to the weight
                    string queryWeightId = "SELECT AnimalId FROM Weight WHERE Name = '" + chart_AnimalId_txt.Value + "'";
                    string weight = Connection.ReadString(queryWeightId);
                    int weightId = Convert.ToInt32(weight);

                    string queryChartWeight = "SELECT AVG(WeightAverage) AS avg_weight FROM Weight WHERE AnimalId = '" + weightId + "' AND Date BETWEEN ''"+ frm_date +"'' AND ''"+ to_date +"'';";
                    LoadChart(queryChartWeight, weight_chart);
                    weight_chart.DataSource = con;
                    weight_chart.DataBind();
                }
            }
        }
